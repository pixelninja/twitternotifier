...
class Extension_TwitterNotifier extends Extension
{
	...
	public function install() {
		return Symphony::Database()->query("
			CREATE TABLE IF NOT EXISTS `".$this->table."` (
				...
				`custom_msg` varchar(100) NOT NULL,
	...
	public function initialiseAdminPageHead($context) {
        ...
        if($page instanceof contentExtensionTwitterNotifierAccounts){
            $page->addStylesheetToHead(URL . '/extensions/twitternotifier/assets/twitternotifier.css', 'screen', 666);
            $page->addScriptToHead(URL . '/extensions/twitternotifier/assets/jquery.oauthpopup.js', 667);
            $page->addScriptToHead(URL . '/extensions/twitternotifier/assets/twitternotifier.js', 667);
        }
	}
	...
	public function sendTwitterNotification($context) {
		...
		foreach($accounts as $account) {
			...
			$url = $account['base_url'] . '/' . ($page['path'] ? $page['path'] . '/' : '') . $page['handle'] . '/' . $account['params'] . '/';            
			$url_handle = $context['entry']->getData($account['field_param']);
			$url = str_replace('$field', $url_handle['handle'], $url);
			$url = preg_replace('#^https?://#', '', $url);

            $prefix = $account['custom_msg'] . ' \'';
			$title = $context['entry']->getData($account['field_msg']);
			...
			$reserve = $reserve + strlen($prefix) + strlen($msg_append);

            if (strlen($title['value']) > 94) {
                $title = substr($title['value'], 0, (141 - $reserve - 3)) . '...';
            }
            else {
                $title = substr($title['value'], 0, (141 - $reserve));
            }

            $tweet = $prefix . $title . $msg_append . $url;
